    ...
    const params = useParams()
      ...
      useEffect(() => {
        async function fetchAssignment() {
          try {
            // Use submission endpoint which returns assignment with submissions
            const res = await fetch(`/api/submission?assignmentId=${id}`);
            if (!res.ok) throw new Error("Failed to fetch assignment");
            const data: Assignment = await res.json();
            setAssignment(data);

            // find current user's submission if available
            if (user && Array.isArray((data as any).submissions)) {
              const mine = (data as any).submissions.find((s: any) => s.studentId === user.userId || s.student?.id === user.userId);
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
                ...
                <div>
                  ...
                  {mySubmission && (
                    <div className="mt-4">
                      <p className="text-sm font-medium">Your existing submission:</p>
                      <div className="text-sm">
                        {(() => {
                          const fileUrl = (mySubmission as any).content || (mySubmission as any).filePath;
                          ...
                          if (lower.endsWith('.pdf')) {
                            return (
                              <div className="border rounded overflow-hidden">
                                <iframe src={fileUrl} className="w-full h-64" title="submission-pdf" />
                              </div>
                            );
                          }

                          if (lower.match(/\.(png|jpe?g|gif|webp)$/)) {
                            return <img src={fileUrl} alt="submission" className="max-w-full h-auto rounded" />;
                          }

                          return <a href={fileUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">Download</a>;
                        })()}
                      </div>
                      <p className="text-sm">Grade: {(mySubmission as any).grade ?? 'Not graded yet'}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <button
              ...
              disabled={submitting}
              onClick={async () => {
                if (!selectedFile) return toast.error('Please select a file to submit');
                setSubmitting(true);
                const form = new FormData();
                form.append('file', selectedFile as File);
                try {
                  if (mySubmission) {
                    // update existing submission via PATCH multipart
                    const res = await fetch(`/api/submission/${mySubmission.id}`, { method: 'PATCH', body: form });
                    const body = await res.json();
                    if (!res.ok) return toast.error(body?.error || 'Failed to update submission');
                    toast.success(body?.message || 'Submission updated');
                    setSelectedFile(null);
                    // refresh
                    const r = await fetch(`/api/submission?assignmentId=${id}`);
                    const d = await r.json();
                    setAssignment(d);
                    const mine = (d as any).submissions.find((s: any) => s.studentId === user?.userId || s.student?.id === user?.userId);
                    setMySubmission(mine || null);
                  } else {
                    const res = await fetch(`/api/submission?assignmentId=${id}`, { method: 'POST', body: form });
                    const body = await res.json();
                    if (!res.ok) return toast.error(body?.error || 'Failed to submit');
                    toast.success(body?.message || 'Submission successful');
                    setSelectedFile(null);
                    const r = await fetch(`/api/submission?assignmentId=${id}`);
                    const d = await r.json();
                    setAssignment(d);
            ...
            >
              {submitting ? (mySubmission ? 'Updating...' : 'Submitting...') : (mySubmission ? 'Update Submission' : 'Submit Assignment')}
            </button>
          </div>
        </div>
      </main>
