...
const ExtensionTable = () => {
  ...
  if (loading) return <p className="text-gray-500">Loading extension requests...</p>;
  if (extensions.length === 0) return <p className="text-gray-500">No extension requests found.</p>;
            ...
            <tbody>
              {extensions.map((item) => (
                <tr key={item.id} className="border-t">
                  <td className="py-3 px-6">{(item.student as any)?.fullname ?? "—"}</td>
                  ...
                  <td className="py-3 px-6">{formatDate(item.requestedDate ?? item.createdAt)}</td>
                    ...
                    {item.status === "PENDING" ? (
                      <>
                        <button
                          ...
                          className={`${
                            actionLoading === item.id ? "bg-green-300" : "bg-green-500 hover:bg-green-600"
                        ...
                        <button
                          ...
                          disabled={actionLoading === item.id}
                          className={`${
                            actionLoading === item.id ? "bg-red-300" : "bg-red-500 hover:bg-red-600"
                          } text-white px-3 py-1 rounded-md text-xs font-medium`}
                        >
